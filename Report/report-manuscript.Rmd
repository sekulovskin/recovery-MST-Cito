...
#### Significant differences observed in the first analyses
...
By now it is already evident the most of the problems concerning the recovery from early mistakes are present in the Lezen MST. However, due to the complexity of the research question, no single way to quantify this "hint" is available. Thus, in an effort to incpect whether significant differences for the theta discrepancy between the true courses are present when controlling for certain covariates, such as the number of mistakes in module A, the number of mistakes in day 1,  the number of different paths taken, and the weights of the items in Modules A, an [Aalysis of Covariance](https://en.wikipedia.org/wiki/Analysis_of_covariance) was performed on the simulated data. First all the variables are explained:

 - **Dependant variable:**: Difference between the absolute values of the true and re-restimated thetas;
